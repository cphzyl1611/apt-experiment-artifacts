...
def _strict_raw_file_access_event(
    bundle: Mapping[str, Any], audit_key: str, watched_path: str,
) -> dict[str, Any] | None:
    """Return one strictly validated file event and its source records."""
...
def _valid_raw_file_access_event(
    ...
    event = _strict_raw_file_access_event(bundle, audit_key, watched_path)
    if event is None:
        return None
    return {
        key: value for key, value in event.items()
        if key not in {"syscall_fields", "syscall_record"}
    }
...
def poll_audit_evidence(
    ...
    while True:
        ...
        if accepted_at < deadline and final_returncode == 0 and events and micro_probe_verdict(events) == "PASS":
            event = events[0]
            return {
                "verdict": "PASS", "audit_serial": event["raw_serial"],
                "evidence_stdout": final_stdout, "poll_attempts": attempts,
                "elapsed_visibility_latency": max(0.0, accepted_at - started),
    ...
    return {
        ...
        "evidence_mode": "RAW",
        "failure_reason": (
            "AUSEARCH_TIMEOUT" if timeout_expired
            else "AUSEARCH_DEADLINE_EXPIRED" if deadline_expired
            else "AUDIT_EVIDENCE_MISSING"
        ),
...
def collect_production_audit_evidence(
    ...
    if audit_raw.get("returncode") != 0:
        raise RuntimeError(
            f"bounded ausearch failed closed: returncode={audit_raw.get('returncode')}"
        )
    return audit_raw
...
def normalize_audit_record(
    record, pid_joins, permission_paths=(), strict_permission_events=None,
):
    ...
    permission_candidate = bool(permission_path_set.intersection(paths)) and syscall in {"openat", "openat2", "read", "write", "pread64", "pwrite64", "readv", "writev", "pwritev2"}
    strict_permission_event = None
    if strict_permission_events is not None:
        strict_permission_event = strict_permission_events.get(record.get("serial"))
        if permission_candidate and strict_permission_event is None:
            return None
    permission_match = strict_permission_event is not None or permission_candidate
    if strict_permission_event is not None:
        parsed.update(strict_permission_event["syscall_fields"])
        syscall = strict_permission_event["underlying_syscall"]
        paths = [strict_permission_event["watched_path"]]
    ...
    if event_type == EVENT_TYPE:
        row.update({
            "evidence_basis": EVIDENCE_BASIS,
            "watched_path": strict_permission_event["watched_path"] if strict_permission_event else next(iter(permission_path_set.intersection(paths)), None),
            "requested_access": requested_access,
            "underlying_syscall": syscall,
        })
    return row
...
def normalize_production_audit_records(raw, audit_key, pid_joins, permission_paths):
    """Normalize full-smoke output only after strict raw file-event validation."""
    permission_paths = [str(path) for path in permission_paths]
    raw_records = parse_audit_groups(raw, audit_key.encode())
    strict_permission_events = {}
    for bundle in parse_raw_audit_event_bundles(raw):
        for path in permission_paths:
            if _valid_raw_file_access_event(bundle, audit_key, path) is not None:
                strict_permission_events[bundle["serial"]] = _strict_raw_file_access_event(
                    bundle, audit_key, path
                )
                break
    normalized = [
        event
...
def collect_production_audit_records(
    audit_key, pid_joins, permission_paths, deadline=None,
    runner=None, monotonic=None,
):
    """Collect and strictly normalize one full-smoke audit response."""
    audit_raw = collect_production_audit_evidence(
        audit_key, deadline=deadline, runner=runner, monotonic=monotonic,
    )
    raw_records, normalized = normalize_production_audit_records(
        audit_raw.get("stdout", b""), audit_key, pid_joins, permission_paths
    )
    return audit_raw, raw_records, normalized
...
def static_self_check():
    ...
    production_ausearch_bounded = (
        ...
        and "def collect_production_audit_records" in source
        and "collect_production_audit_records(" in source
...
def _run_reviewed_mininet_smoke():
    ...
    try:
        ...
        audit_raw, raw_records, normalized = collect_production_audit_records(
            key, pid_joins, permission_paths.values()
        )
