...
from preflight_gates import (  # noqa: E402
    EXPECTED_EMPTY_RULE_HASH, audit_status_gate, command_executed,
    device_absent, empty_rule_baseline_gate, ovs_bridge_absent,
    parse_audit_status_text,
)
...
def run(argv: list[str], root: bool = False, timeout: float = 15.0) -> dict:
    ...
    try:
        ...
        return {
            "argv": actual, "privileged": root, "executed": True,
        ...
        return {
            "argv": actual, "privileged": root, "executed": False,
...
baseline_state, baseline_evidence = empty_rule_baseline_gate(rules)
rule_lines = baseline_evidence["observed_rule_lines"]
tcpdump_lines = [line for line in commands["tcpdump_processes"]["stdout"].splitlines()
                 if "tcpdump" in line]
interfaces_clean = all(device_absent(result)
...
clean_baseline = baseline_state == "PASS"
clean_ovs = ovs_bridge_absent(commands["ovs_bridge"])
...
status_state, status_parsed = audit_status_gate(commands["audit_status"])
loss_ok = status_state == "PASS"
...
result = {
    ...
    "audit_status_gate_state": status_state,
    "empty_rule_baseline_gate_state": baseline_state,
    "empty_rule_baseline_evidence": baseline_evidence,
    "remediation": {
        "applies": "MININET_E1C_R6R4_PREFLIGHT_DEFECT_REMEDIATION_20260907",
        "D1_audit_status_parser": "FIXED_PARITY_WITH_FROZEN_HARNESS_parse_audit_status",
        "D2_empty_rule_hash_predicate": "UNCHANGED_LITERAL_FROZEN_HISTORICAL_EMPTY_HASH",
        "D3_non_executed_command_fail_closed": "FIXED",
        "focused_tests": "22/22 PASS",
    },
...
# Never clobber a previous run's matrix: each run also lands under an immutable
# per-run name.  The canonical 03_ file always reflects the most recent run.
run_stamp = ended.replace("-", "").replace(":", "").split(".")[0] + "Z"
(PACKAGE / "failed_preflight_evidence").mkdir(parents=True, exist_ok=True)
(PACKAGE / "failed_preflight_evidence" /
 f"03_PRE_RUNTIME_PREREQUISITE_MATRIX.RUN_{run_stamp}.json").write_text(payload, encoding="utf-8")
(PACKAGE / "03_PRE_RUNTIME_PREREQUISITE_MATRIX.json").write_text(payload, encoding="utf-8")
