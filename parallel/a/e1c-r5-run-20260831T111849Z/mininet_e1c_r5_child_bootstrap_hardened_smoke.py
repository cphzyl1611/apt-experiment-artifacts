#!/usr/bin/env python3
"""Bounded E1C-R5 auditd smoke harness with child bootstrap diagnostics.
...
PCAP_PATH = RUN_DIR / "MININET_E1C_R5_SMOKE.pcap"

PRE_STATE_PATH = RUN_DIR / "MININET_E1C_R5_AUDIT_PRE_STATE.json"
LINEAGE_PATH = RUN_DIR / "MININET_E1C_R5_LINEAGE.json"
STATIC_AUDIT_PATH = RUN_DIR / "MININET_E1C_R5_STATIC_AUDIT.json"
PRE_RUN_CONTRACT_PATH = RUN_DIR / "MININET_E1C_R5_PRE_RUN_CONTRACT.json"
JOURNAL_PATH = RUN_DIR / "MININET_E1C_R5_RULE_REMEDIATION_JOURNAL.jsonl"
RESIDUAL_PATH = RUN_DIR / "MININET_E1C_R5_RESIDUAL_REMEDIATION.json"
RULE_CONTRACT_PATH = RUN_DIR / "MININET_E1C_R5_TRANSIENT_RULE_CONTRACT.json"
RAW_PATH = RUN_DIR / "MININET_E1C_R5_RAW_AUDIT_EVIDENCE.jsonl"
NORMALIZED_PATH = RUN_DIR / "MININET_E1C_R5_NORMALIZED_EVENTS.jsonl"
JOIN_PATH = RUN_DIR / "MININET_E1C_R5_PID_NETNS_JOIN.jsonl"
COVERAGE_PATH = RUN_DIR / "MININET_E1C_R5_COVERAGE_AND_LOSS.json"
POST_PATH = RUN_DIR / "MININET_E1C_R5_POST_CLEANUP.json"
STRACE_PATH = RUN_DIR / "MININET_E1C_R5_STRACE_ORACLE_COMPARISON.json"
REPORT_PATH = RUN_DIR / "MININET_E1C_R5_AUDITD_SMOKE_REPORT.md"
RESULT_PATH = RUN_DIR / "MININET_E1C_R5_PRIVILEGED_RUN_RESULT.json"
EARLY_FAILURE_PATH = RUN_DIR / "MININET_E1C_R5_EARLY_CHILD_FAILURES.jsonl"
CHILD_STATE_PATH = RUN_DIR / "MININET_E1C_R5_CHILD_STATE_TRANSITIONS.jsonl"
CHILD_PROCESS_PATH = RUN_DIR / "MININET_E1C_R5_CHILD_PROCESS_EVIDENCE.json"
...
def remediate_residual_rules(pre, journal_path):
    state = {
        "schema": "MININET_E1C_R5_RESIDUAL_REMEDIATION_V1", "run_id": RUN_ID,
...
def build_rule_specs(key, pid, file_dir, supported, ppid=None, scope="pid"):
    ...
    for name, candidates, extra in groups:
        for syscall in candidates:
            ...
            specs.append({"name": f"{scope}_{name}_{syscall}", "syscall": syscall, "add_argv": argv, "subject": subject, "source": "R5_SMOKE"})
    return specs
...
def query_supported_syscalls(probe_key, journal_path):
    ...
    for syscall in SYSCALL_CANDIDATES:
        spec = {"name": f"probe_{syscall}", "add_argv": ["/usr/sbin/auditctl", "-a", "always,exit", "-F", "arch=b64", "-S", syscall, "-F", f"pid={os.getpid()}", "-k", probe_key], "source": "R5_SYSCALL_PROBE"}
        add_result = mutation(journal_path, spec, "ADD", "R5_PROBE")
        remove_result = None
        if add_result.get("returncode") == 0:
            supported.add(syscall)
            remove_result = mutation(journal_path, spec, "DELETE", "R5_PROBE")
...
def raise_child_protocol_error(path, logical_host_id, unexpected_event, child_pid,
    ...
    """
    evidence = {
        "schema": "MININET_E1C_R5_EARLY_CHILD_FAILURE_V1",
...
def persist_state_transition(path, logical_host_id, state, pid=None, stage=None,
                             **fields):
    row = {
        "schema": "MININET_E1C_R5_CHILD_STATE_V1", "timestamp_utc": utc_now(),
...
def benign_child(args):
    ...
    try:
        ...
        with temp.open("a", encoding="utf-8") as stream:
            stream.write("r5-write\n")
        operations.append("write")
        temp.read_text(encoding="utf-8")
        operations.append("read")
        transition("FILE_OPS_COMPLETE", file_operations=list(operations), temp_file=str(temp))
        ...
        ready = {
            ...
            "temp_file": str(temp), "file_operations": list(operations),
...
def collect_child_process_evidence(children):
    ...
    write_json_atomic(CHILD_PROCESS_PATH, {"schema": "MININET_E1C_R5_CHILD_PROCESS_EVIDENCE_V1", "children": rows})
    state_rows = []
...
def static_self_check():
    ...
    result = {
        "schema": "MININET_E1C_R5_STATIC_AUDIT_V1", "checked_at_utc": utc_now(), "harness_path": str(HARNESS_PATH),
...
def verify_clean_root_baseline(baseline):
    """R5 never performs legacy cleanup; a non-empty root baseline fails closed."""
    return {
        "status": "PASS" if (
        ...
        "audit_baseline_restored_before_r5": not baseline.get("canonical_rule_lines") and baseline.get("baseline_rule_dump_sha256") == HISTORICAL_EMPTY_HASH,
        "AUDIT_BASELINE_RESTORED_BEFORE_R5": "YES" if (not baseline.get("canonical_rule_lines") and baseline.get("baseline_rule_dump_sha256") == HISTORICAL_EMPTY_HASH) else "NO",
        "legacy_cleanup_performed": False,
        "baseline": baseline,
    }
...
def privileged_run():
    ...
    key = "e1c5" + hashlib.sha256((RUN_ID + utc_now()).encode()).hexdigest()[:12]
    probe_key = "e1c5probe" + hashlib.sha256((RUN_ID + "probe").encode()).hexdigest()[:10]
    ...
    write_json_atomic(STRACE_PATH, {"schema": "MININET_E1C_R5_STRACE_ORACLE_COMPARISON_V1", "status": "NOT_RUN", "missing_classes_inferred_from_strace": False})
    try:
        ...
        write_json_atomic(RULE_CONTRACT_PATH, {"schema": "MININET_E1C_R5_TRANSIENT_RULE_CONTRACT_V1", "run_id": RUN_ID, "audit_key": key, "probe_key": probe_key, "status": "NOT_STARTED", "rule_specs": [], "successful_adds": []})
        ...
        for host in ("h1", "h2"):
            for spec in build_rule_specs(key, children[host]["pid"], str(TEMP_DIR), supported, scope=f"{host}_pid"):
                outcome = mutation(JOURNAL_PATH, spec, "ADD", "R5_SMOKE")
        ...
        for host in ("h1", "h2"):
            for spec in build_rule_specs(key, children[host]["pid"], str(TEMP_DIR), supported, ppid=children[host]["pid"], scope=f"{host}_ppid"):
                ...
                outcomes.append(entry)
                if entry["installed"]: installed.append(entry)
        write_json_atomic(RULE_CONTRACT_PATH, {"schema": "MININET_E1C_R5_TRANSIENT_RULE_CONTRACT_V1", "run_id": RUN_ID, "audit_key": key, "probe_key": probe_key, "status": "ACTIVE", "supported_syscalls": sorted(supported), "syscall_probes": syscall_probes, "rule_specs": outcomes, "successful_adds": [item["name"] for item in installed]})
        ...
        write_jsonl_atomic(RAW_PATH, [{"schema": "MININET_E1C_R5_RAW_AUDIT_EVIDENCE_V1", "run_id": RUN_ID, "audit_key": key, **{k: v for k, v in record.items() if k not in {"raw_bytes", "raw_text"}}, "raw_text": record["raw_text"]} for record in raw_records])
        ...
        post = {
            "schema": "MININET_E1C_R5_POST_CLEANUP_V1", "run_id": RUN_ID, "audit_key": key,
            ...
            "baseline_restored_after_r5": not rules_after.get("canonical_rule_lines") and rules_after.get("baseline_rule_dump_sha256") == HISTORICAL_EMPTY_HASH,
            "AUDIT_BASELINE_RESTORED_AFTER_R5": "YES" if (not rules_after.get("canonical_rule_lines") and rules_after.get("baseline_rule_dump_sha256") == HISTORICAL_EMPTY_HASH) else "NO",
        ...
        coverage = {
            "schema": "MININET_E1C_R5_COVERAGE_AND_LOSS_V1", "run_id": RUN_ID, "audit_key": key,
        ...
        coverage["MININET_E1C_R5_AUDITD_COLLECTOR"] = classification
        write_json_atomic(COVERAGE_PATH, coverage)
        write_json_atomic(RULE_CONTRACT_PATH, {"schema": "MININET_E1C_R5_TRANSIENT_RULE_CONTRACT_V1", "run_id": RUN_ID, "audit_key": key, "probe_key": probe_key, "status": "REMOVED" if post["run_rules_removed"] else "REMOVAL_FAILED", "supported_syscalls": sorted(supported), "syscall_probes": syscall_probes, "rule_specs": outcomes, "successful_add_count": len(installed), "removal": removals})
        result = {"schema": "MININET_E1C_R5_PRIVILEGED_RUN_RESULT_V1", "run_id": RUN_ID, "status": "COMPLETED" if runtime_error is None else "FAILED", "classification": classification, "runtime_error": runtime_error, "formal_experiment_executed": False}
        write_json_atomic(RESULT_PATH, result)
        report = ["# MININET-E1C-R5 Auditd Bounded Smoke", "", f"Run: `{RUN_ID}`", f"Audit key: `{key}`", "", f"`MININET_E1C_R5_AUDITD_COLLECTOR = {classification}`", "", f"`OLD_RUN_RESIDUAL_RULES_FOUND = {remediation.get('old_run_residual_rules_found', 0)}`", f"`OLD_RUN_RESIDUAL_RULES_REMOVED = {remediation.get('old_run_residual_rules_removed', 0)}`", f"`AUDIT_BASELINE_RESTORED_BEFORE_R5 = {'YES' if remediation.get('audit_baseline_restored_before_r5') else 'NO'}`", f"`AUDIT_BASELINE_RESTORED_AFTER_R5 = {'YES' if post['baseline_restored_after_r5'] else 'NO'}`", f"`AUDIT_LOST_EVENTS = {coverage.get('audit_lost_events')}`", f"`LOGICAL_HOST_JOIN_SUCCESS_COUNT = {coverage['pid_netns_join_success_count']}`", f"`LOGICAL_HOST_JOIN_FAILURE_COUNT = {coverage['pid_netns_join_failure_count']}`", f"`EARLY_CHILD_FAILURE_COUNT = {coverage.get('early_child_failure_count')}`", "", "## Required audit classes", ""]
