# R6R3 Exact Path Semantics R2R1
...
basename, directory selector, or path pattern. A path or root beginning with
`//` is rejected as ambiguous rather than normalized. `.` and `..` components
are forbidden in the declared string, even when lexical normalization would
place the result below the root. The root comparison is component-wise, so
