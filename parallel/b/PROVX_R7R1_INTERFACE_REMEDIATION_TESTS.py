...
def _fixture(temp: Path, *, event: dict | None = None, join: dict | None = None):
    ...
    paths["result"].write_text(json.dumps({"run_id": R6_RUN_ID, "status": "COMPLETED", "classification": "PASS"}) + "\n")
...
class R7R1InterfaceRemediationTests(unittest.TestCase):
    ...
    def test_file_rw_maps_to_host_scoped_file_write(self):
        ...
        self.assertTrue(graph.edge_map[0]["dst_graph_node_id"].startswith("path:/tmp/r6-bound-file"))
        self.assertEqual(graph.encoder_records[0]["destination"]["type"], "FILE")

    def test_file_rw_missing_permission_evidence_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp:
            _, event, join = _fixture(Path(temp), event={"evidence_basis": None, "watched_path": None, "requested_access": None, "underlying_syscall": None})
            graph = build_graph([event], [join], run_id=R6_RUN_ID)
