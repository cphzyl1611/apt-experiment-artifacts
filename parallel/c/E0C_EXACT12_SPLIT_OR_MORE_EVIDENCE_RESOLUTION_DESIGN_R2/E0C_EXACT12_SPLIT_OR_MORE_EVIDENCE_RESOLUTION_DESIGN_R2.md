...
R2 remediates only the two blockers identified in the R1 independent design
review:

1. strict split-proposal validation for frozen identity and partition
   conservation;
2. explicit `PARTITION` and `GOVERNANCE` evidence acquisition classes.

R1 remains the prior design input. The R2 package does not apply a split, make a
human decision, mutate a status, change the denominator, authorize execution,
run an experiment, or push to a remote repository.
...
A proposal passes only when both gates return zero errors. Submitted booleans,
counts, and hashes cannot override recomputation.
