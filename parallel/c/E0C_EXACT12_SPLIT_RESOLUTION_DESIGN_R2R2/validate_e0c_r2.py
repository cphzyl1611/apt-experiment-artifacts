...
REFERENCE_PATTERN = re.compile(GOVERNANCE_REFERENCE_PATTERN)
...
def validate_governance_reference_syntax_domain(
    syntax_fixture: Mapping[str, Any], schema: Mapping[str, Any]
) -> dict[str, Any]:
    schema_pattern = schema["$defs"]["reference"]["pattern"]
    schema_reference_pattern = re.compile(schema_pattern)
    accepted = syntax_fixture["accepted"]
    rejected = syntax_fixture["rejected"]
    return {
        "schema_pattern": schema_pattern,
        "semantic_pattern": GOVERNANCE_REFERENCE_PATTERN,
        "patterns_identical": schema_pattern == GOVERNANCE_REFERENCE_PATTERN,
        "accepted": [
            {
                "reference": reference,
                "schema_accepts": schema_reference_pattern.fullmatch(reference) is not None,
                "semantic_accepts": REFERENCE_PATTERN.fullmatch(reference) is not None,
            }
        ...
        "rejected": [
            {
                "reference": reference,
                "schema_rejects": schema_reference_pattern.fullmatch(reference) is None,
                "semantic_rejects": REFERENCE_PATTERN.fullmatch(reference) is None,
            }
...
def build_evidence(package_dir: Path = PACKAGE_DIR) -> dict[str, Any]:
    ...
    governance_reference_syntax = validate_governance_reference_syntax_domain(
        load_json(package_dir / "fixtures" / "GOVERNANCE_REFERENCE_SYNTAX_WITNESSES.json"),
        schema,
    )
    boundary = load_json(package_dir / "ZERO_MUTATION_BOUNDARY.json")
    governance_reference_syntax_passes = (
        governance_reference_syntax["patterns_identical"]
        and all(
            item["schema_accepts"] and item["semantic_accepts"]
            for item in governance_reference_syntax["accepted"]
        )
        and all(
            item["schema_rejects"] and item["semantic_rejects"]
            for item in governance_reference_syntax["rejected"]
        )
    )
    ...
    terminal = {
        "E0C_EXACT12_SPLIT_RESOLUTION_DESIGN_R2R2": "PASS_READY_FOR_INDEPENDENT_REVIEW" if baseline.valid and all_fixture_checks_pass and governance_reference_syntax_passes and {"PARTITION", "GOVERNANCE"}.issubset(classes) else "BLOCKED",
        ...
        "GOVERNANCE_REFERENCE_SYNTAX_ALIGNMENT": "PASS" if governance_reference_syntax_passes else "BLOCKED",
    ...
    return {
        "artifact_type": "E0C_EXACT12_SPLIT_RESOLUTION_DESIGN_R2R2_VALIDATION_EVIDENCE",
        "read_only": True,
        "schema_meta_validation": "PASS",
        ...
        "governance_reference_syntax": governance_reference_syntax,
        "human_decisions_created": 0,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write_evidence:
        ...
        (PACKAGE_DIR / "E0C_EXACT12_SPLIT_RESOLUTION_DESIGN_R2R2_VALIDATION_EVIDENCE.json").write_text(
