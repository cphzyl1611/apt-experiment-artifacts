...
def authority_graph():
    entries = [common_entry(f"synthetic-r3-root-{index}", role, f"root-{index}\n".encode()) for index, role in enumerate(ROLES)]
...
class R3DefectRegressionTests(unittest.TestCase):
    ...
    def test_b2_omitted_and_extra_tuples_are_rejected(self):
        entry = common_entry("synthetic-r3-populated", ROLES[0], b'[{"authorized":"real"}]')
    ...
    def test_b3_current_derivation_binds_proof_and_rejects_stale_or_wrong_graph(self):
        proof = source_admission_zero_proof(input_set_id="current-input", contract_id="current-contract")
        current = __import__("tests.test_ec_b3_zero_proof", fromlist=["derivation"]).derivation("current-input", "current-contract")
        ...
        stale_context = __import__("tests.test_ec_b3_zero_proof", fromlist=["derivation"]).derivation("old-input", "current-contract")
        ...
        wrong_contract = __import__("tests.test_ec_b3_zero_proof", fromlist=["derivation"]).derivation("current-input", "wrong-contract")
        with self.assertRaisesRegex(ContractError, "STALE_SOURCE_ADMISSION_ZERO_PROOF"):
            validate_source_admission_zero_proof_freshness(proof, wrong_contract)
        wrong_evidence = copy.deepcopy(current)
        wrong_evidence["machine_authority_evaluation_evidence"] = copy.deepcopy(current["machine_authority_evaluation_evidence"])
        wrong_evidence["machine_authority_evaluation_evidence"]["evaluation_run_input_identity"] = "wrong-run"
        wrong_evidence["machine_authority_evaluation_evidence"]["machine_authority_evaluation_evidence_id"] = __import__("tests.fixtures", fromlist=["with_id"]).with_id(wrong_evidence["machine_authority_evaluation_evidence"], "machine_authority_evaluation_evidence_id")["machine_authority_evaluation_evidence_id"]
        with self.assertRaisesRegex(ContractError, "STALE_SOURCE_ADMISSION_ZERO_PROOF"):
            validate_source_admission_zero_proof_freshness(proof, wrong_evidence)
    ...
    def test_b4_exact_set_is_derived_and_subsets_supersets_duplicates_rejected(self):
        def make(kind, values, context):
            ...
            vector = [{"target_id": target, "result_commitment": value} for target, value in values]
        ...
        primary = make("primary_commitment", [("t1", "a" * 64), ("t2", "b" * 64)], "primary-context")
        verifier = make("verifier_commitment", [("t1", "x" * 64), ("t2", "b" * 64)], "verifier-context")
        self.assertEqual(compare_commitments(primary, verifier, True, affected_target_ids=["t1"])["affected_target_ids"], ["t1"])
        for asserted in (["t2"], [], ["t1", "t2"], ["unrelated"], ["t1", "t1"]):
            with self.assertRaisesRegex(ContractError, "AFFECTED_TARGET_LIST_INCONSISTENT"):
                compare_commitments(primary, verifier, True, affected_target_ids=asserted)
        equal = make("verifier_commitment", [("t1", "a" * 64), ("t2", "b" * 64)], "equal-verifier")
        with self.assertRaisesRegex(ContractError, "AFFECTED_TARGET_LIST_INCONSISTENT"):
            compare_commitments(primary, equal, True, affected_target_ids=["t1"])
        with self.assertRaisesRegex(ContractError, "AFFECTED_TARGET_LIST_INCONSISTENT"):
