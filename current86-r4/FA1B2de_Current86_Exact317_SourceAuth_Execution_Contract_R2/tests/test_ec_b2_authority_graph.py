...
from tests.fixtures import authority_expansion, common_entry, common_manifest, frozen_authority, with_id
...
def graph():
    ...
    return manifest, authorities, expansions
...
class AuthenticatedAuthorityGraphTests(unittest.TestCase):
    ...
    def test_valid_synthetic_graph_derives_status_and_ordered_input_set(self):
        manifest, authorities, expansions = graph()
        context = derive_machine_authority_context(manifest, authorities, expansions, ROLES, CONTRACT_ID)
    ...
    def test_caller_authenticated_boolean_without_frozen_record_is_rejected(self):
        ...
        with self.assertRaisesRegex(ContractError, "CALLER_TRUSTED_STATUS_REJECTED"):
            derive_machine_authority_context(manifest, caller_summary, expansions, ROLES, CONTRACT_ID)
    ...
    def test_missing_root_is_not_zero(self):
        ...
        with self.assertRaisesRegex(ContractError, "MISSING_AUTHORITY_ROOT_NOT_ZERO"):
            derive_machine_authority_context(manifest, authorities[:-1], expansions[:-1], ROLES, CONTRACT_ID)
