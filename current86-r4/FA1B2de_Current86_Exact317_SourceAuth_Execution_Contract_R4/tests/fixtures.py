...
EVALUATOR_SHA256 = hashlib.sha256((Path(__file__).resolve().parents[1] / "tools" / "frozen_authority_evaluator.py").read_bytes()).hexdigest()
...
def common_entry(logical_id="synthetic-artifact-a", role="FIELD_PIN_REGISTRY", content=b"[]"):
    ...
    return with_id({
        ...
        "available": True,
        "content_opened": True,
        "authenticated_artifact_bytes": content.decode("utf-8"),
...
def authority_expansion(authority, tuples=None, evaluation_contract_id="synthetic-evaluation-contract-r2", complete=True):
    ...
    return with_id({
        "schema": "FA1B2DE_CURRENT86_MACHINE_AUTHORITY_EXPANSION_RECORD_R3",
        "audit_scope_id": AUDIT_SCOPE_ID,
        "authority_record_id": authority["authority_record_id"],
        "common_input_entry_id": authority["common_input_entry_id"],
        "artifact_content_identity": authority["sha256_or_explicit_pinned_identity"],
        "evaluator_implementation_id": "FA1B2DE_FROZEN_MACHINE_AUTHORITY_EVALUATOR_R4",
        "evaluator_implementation_sha256": EVALUATOR_SHA256,
        "evaluator_configuration_id": "FA1B2DE_MACHINE_AUTHORITY_EVALUATOR_CONFIG_R4",
        "evaluation_contract_id": evaluation_contract_id,
        "evaluation_run_input_identity": "synthetic-evaluation-run-input-r3",
        "evaluation_evidence_id": "synthetic-evaluation-evidence-r3",
...
def authority_evidence(authorities, expansions, input_set_id="synthetic-authority-input-set-r3", evaluation_contract_id="synthetic-evaluation-contract-r2"):
    ordered = sorted(authorities, key=lambda row: (row["authority_role"].encode("utf-8"), row["logical_artifact_id"].encode("utf-8")))
    by_id = {row["authority_record_id"]: row for row in expansions}
    if input_set_id == "synthetic-authority-input-set-r3":
        input_basis = [{
        ...
        input_set_id = digest(input_basis)
    ordered_expansions = [by_id[row["authority_record_id"]] for row in ordered]
    return with_id({
        ...
        "evaluation_run_input_identity": "synthetic-evaluation-run-input-r3",
        "evaluation_evidence_id": "synthetic-evaluation-evidence-r3",
        "authority_record_ids": [row["authority_record_id"] for row in ordered],
        "ordered_expansion_record_ids": [row["expansion_record_id"] for row in ordered_expansions],
        "complete_input_open_audit": ["synthetic-complete-input-open-audit-r3"],
        "authority_authentication_results": ["PASS"] * len(ordered),
        "deterministic_expansion_outputs": [row["ordered_tuple_ids"] for row in ordered_expansions],
        "rejected_record_reason_codes": [],
        "ordered_set_commitments": [row["ordered_set_commitment_id"] for row in ordered_expansions],
...
def commitment(kind="primary_commitment", result_hash="1" * 64, common_id="synthetic-common-input-r2", role=None, context="context-p", run="run-p"):
    ...
    target_manifest = json.loads((Path(__file__).resolve().parents[1] / "00_lineage" / "EXACT317_TARGET_MANIFEST.json").read_text(encoding="utf-8"))
    target_ids = [row["source_binding_target_id"] for row in target_manifest["targets"]]
    vector = [{"target_id": target_id, "result_commitment": result_hash} for target_id in target_ids]
    return with_id({
        "schema": "FA1B2DE_CURRENT86_SOURCE_AUTH_PRIMARY_COMMITMENT_R3" if kind == "primary_commitment" else "FA1B2DE_CURRENT86_SOURCE_AUTH_VERIFIER_COMMITMENT_R3",
        "role": role,
        ...
        "ordered_target_result_vector": vector,
        "ordered_result_vector_sha256": digest(vector),
        "terminal_state_count_map": {"BLOCKED_FIELD_PIN": len(target_ids)},
        "exact_target_id_set_sha256": digest(target_ids),
