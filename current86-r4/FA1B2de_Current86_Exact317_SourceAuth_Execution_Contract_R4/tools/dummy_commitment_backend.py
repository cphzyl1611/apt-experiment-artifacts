...
target_manifest = json.loads((common / "exact317_target_manifest.json").read_text(encoding="utf-8"))
target_ids = [item["source_binding_target_id"] for item in target_manifest["targets"]]
vector = [{"target_id": target_id, "result_commitment": "1" * 64} for target_id in target_ids]
row = {
    "schema": "FA1B2DE_CURRENT86_SOURCE_AUTH_PRIMARY_COMMITMENT_R3" if role == "PRIMARY" else "FA1B2DE_CURRENT86_SOURCE_AUTH_VERIFIER_COMMITMENT_R3",
    "role": role,
