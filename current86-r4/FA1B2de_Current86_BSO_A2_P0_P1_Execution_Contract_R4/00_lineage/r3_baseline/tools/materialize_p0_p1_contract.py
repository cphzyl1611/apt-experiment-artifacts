...
def _universe_pairs(universe: dict[str, Any] | None) -> set[tuple[str, str]] | None:
    if universe is None:
        return None
    rows = universe.get("candidate_relations") if isinstance(universe, dict) else None
    if not isinstance(rows, list) or not rows:
        raise ValueError("authenticated candidate universe is missing")
    pairs = {(row.get("candidate_scoring_id"), row.get("relation_identity")) for row in rows}
    if any(not isinstance(c, str) or not isinstance(r, str) for c, r in pairs) or len(pairs) != len(rows):
        raise ValueError("authenticated candidate universe is malformed")
    return pairs


def validate_human_decision(packet: dict[str, Any], decision: dict[str, Any], comparison: dict[str, Any] | None = None) -> bool:
    ...
    if action == "CONFIRM_PROPOSED_OWNER":
        ...
        if comparison is not None and (comparison.get("selected_candidate_scoring_id"), comparison.get("selected_relation_identity")) != (selected_candidate, selected_relation):
...
def validate_human_packet(packet: dict[str, Any], frozen_candidate_universe: dict[str, Any] | None = None, comparison: dict[str, Any] | None = None) -> bool:
    ...
    universe_pairs = _universe_pairs(frozen_candidate_universe)
    if universe_pairs is not None:
    ...
    if comparison is not None and (comparison.get("selected_candidate_scoring_id"), comparison.get("selected_relation_identity")) != (packet.get("proposed_candidate_scoring_id"), packet.get("proposed_relation_identity")):
        raise ValueError("human packet proposal does not equal authenticated comparison result")
...
def validate_owner_terminal(record: dict[str, Any], human_decision: dict[str, Any] | None = None, human_packet: dict[str, Any] | None = None, comparison: dict[str, Any] | None = None, frozen_candidate_universe: dict[str, Any] | None = None) -> bool:
    ...
    if any(value is not None for value in (human_decision, human_packet, comparison, frozen_candidate_universe)):
        if not all(value is not None for value in (human_decision, human_packet, comparison, frozen_candidate_universe)):
            raise ValueError("owner terminal cross-object references are incomplete")
        validate_human_packet(human_packet, frozen_candidate_universe, comparison)
        validate_human_decision(human_packet, human_decision, comparison)
        ...
        if record.get("human_action") == "REJECT_PROPOSAL_AND_SELECT_OTHER_EXISTING_CANDIDATE":
            ...
            if isinstance(verification, dict):
                if verification.get("status") != "PASS" or (verification.get("candidate_scoring_id"), verification.get("relation_identity")) != (record.get("selected_owner_candidate_scoring_id"), record.get("selected_relation_identity")):
                    raise ValueError("alternative verification does not bind exact selected alternative")
    if record.get("human_action") == "REJECT_PROPOSAL_AND_SELECT_OTHER_EXISTING_CANDIDATE":
        verification = record.get("fresh_alternative_verification")
...
def validate_disposition_record(record: dict[str, Any], references: dict[str, Any] | None = None) -> bool:
    ...
    if references is not None:
        ...
        for collection, field in (("pending_terminals", "referenced_pending_terminal_id"), ("reviews", "referenced_review_id"), ("accepted_terminals", "referenced_accepted_terminal_id"), ("remediations", "remediation_reference"), ("attempts", "referenced_attempt_id")):
            identifier = record.get(field)
            if identifier is None:
                continue
            obj = _resolve_reference(references, collection, identifier)
            if obj is None or obj.get("raw_key") != raw or _reference_identifier(obj, field) != identifier:
                raise ValueError(f"{field} does not resolve to exact same-raw referenced object")
        typ = record.get("disposition_type")
        if typ in {"PENDING_TERMINAL_INVALIDATED", "TERMINAL_ACCEPTED"}:
            current = references.get("current_pending_terminal")
            pending = _resolve_reference(references, "pending_terminals", record.get("referenced_pending_terminal_id"))
            if not isinstance(current, dict) or pending is None or current.get("id") != record.get("referenced_pending_terminal_id") or pending.get("id") != current.get("id"):
                raise ValueError("referenced pending terminal is not the exact current pending terminal")
            review = _resolve_reference(references, "reviews", record.get("referenced_review_id"))
            if review is None or review.get("status") not in {"BLOCKING", "PASS", "APPROVES"}:
                raise ValueError("referenced review is not the exact substantive review")
        if typ == "TERMINAL_ACCEPTED":
            accepted = _resolve_reference(references, "accepted_terminals", record.get("referenced_accepted_terminal_id"))
            if accepted is None or accepted.get("id") != record.get("referenced_accepted_terminal_id"):
                raise ValueError("accepted terminal reference does not resolve")
            review = _resolve_reference(references, "reviews", record.get("referenced_review_id"))
            if review.get("approved_terminal_id") != accepted.get("id"):
                raise ValueError("review does not approve referenced accepted terminal")
        if typ == "REMEDIATION_RESTARTED":
            remediation = _resolve_reference(references, "remediations", record.get("remediation_reference"))
            preceding = references.get("preceding_blocked_attempt", references.get("preceding_disposition"))
            preceding_id = _reference_identifier(preceding, "prior_disposition_id") if isinstance(preceding, dict) else None
            if remediation is None or not isinstance(preceding, dict) or record.get("prior_disposition_id") != preceding_id or remediation.get("raw_key") != raw or remediation.get("authorizes_restart_of") != preceding_id:
...
def validate_disposition_chain(records: list[dict[str, Any]], references: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for index, record in enumerate(records):
        validate_disposition_record(record, references)
        typ = record.get("disposition_type")
