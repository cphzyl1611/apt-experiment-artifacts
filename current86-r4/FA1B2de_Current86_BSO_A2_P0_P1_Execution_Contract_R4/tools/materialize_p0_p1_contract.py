...
def _validate_packet_references(
    ...
    for name, value in (("candidate universe", universe), ("proposal", proposal), ("primary commitment", primary), ("verifier commitment", verifier), ("comparison", comparison)):
    ...
    if packet.get("complete_candidate_universe_hash") != _object_hash(universe):
        raise ValueError("human packet candidate universe binding mismatch")
    proposed_pair = (packet.get("proposed_candidate_scoring_id"), packet.get("proposed_relation_identity"))
    for name, value in (("proposal", proposal), ("primary commitment", primary), ("verifier commitment", verifier), ("comparison", comparison)):
    ...
    if comparison.get("primary_commitment_id") != primary_id or comparison.get("verifier_commitment_id") != verifier_id or comparison.get("independent_commitment_comparison") != "PASS":
...
def validate_human_packet(
    ...
    universe_pairs = _validate_packet_references(packet, frozen_candidate_universe, proposal, primary_commitment, verifier_commitment, comparison, authenticated_evidence_references)
...
def validate_human_decision(
    ...
    *,
    ...
    validate_human_packet(packet, frozen_candidate_universe, proposal, primary_commitment, verifier_commitment, comparison, authenticated_evidence_references)
    ...
    if action == "CONFIRM_PROPOSED_OWNER":
        ...
        if (comparison.get("selected_candidate_scoring_id"), comparison.get("selected_relation_identity")) != (selected_candidate, selected_relation):
            raise ValueError("human confirmation does not equal authenticated comparison result")
        return True
...
def validate_disposition_chain(records: list[dict[str, Any]], references: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for index, record in enumerate(records):
        record_references = dict(references or {})
        if record.get("disposition_type") in {"PENDING_TERMINAL_INVALIDATED", "TERMINAL_ACCEPTED", "REMEDIATION_RESTARTED"}:
            ...
            record_references["current_prior_disposition"] = records[index - 1]
        validate_disposition_record(record, record_references or None)
        typ = record.get("disposition_type")
