...
class DispositionTests(unittest.TestCase):
    ...
    def test_valid_chain_and_stale_pending_exclusion(self):
        ...
        pending = {"id": r2["referenced_pending_terminal_id"], "raw_key": raw}
        review = {"id": r3["referenced_review_id"], "raw_key": raw, "status": "BLOCKING", "referenced_pending_terminal_id": r3["referenced_pending_terminal_id"]}
        graph = {"by_raw": {raw: {
    ...
    def test_accepted_terminal_supersedes_historical_blocked_state(self):
        ...
        r2 = self._record(raw, 2, "ATTEMPT_BLOCKED", r1["disposition_id"],
                          "IN_PROGRESS_OR_INCOMPLETE", "BLOCKED_ATTEMPT", reason="TECHNICAL_FAILURE", attempt="attempt-1")
        r3 = self._record(raw, 3, "REMEDIATION_RESTARTED", r2["disposition_id"],
                          "BLOCKED_ATTEMPT", "IN_PROGRESS_OR_INCOMPLETE", attempt="attempt-1", remediation="remediation")
        ...
        r5 = self._record(raw, 5, "TERMINAL_ACCEPTED", r4["disposition_id"],
                          ...
                          accepted="accepted-1")
        attempt = {"id": r2["referenced_attempt_id"], "raw_key": raw, "state": "BLOCKED_ATTEMPT", "disposition_id": r2["disposition_id"]}
        remediation = {"id": r3["remediation_reference"], "raw_key": raw, "authorizes_restart_of": r2["disposition_id"], "authorizes_restart_of_attempt_id": r3["referenced_attempt_id"]}
        pending = {"id": r4["referenced_pending_terminal_id"], "raw_key": raw}
        review = {"id": r5["referenced_review_id"], "raw_key": raw, "status": "PASS", "referenced_pending_terminal_id": r5["referenced_pending_terminal_id"], "approved_terminal_id": r5["referenced_accepted_terminal_id"]}
        accepted = {"id": r5["referenced_accepted_terminal_id"], "raw_key": raw, "referenced_pending_terminal_id": r5["referenced_pending_terminal_id"], "referenced_review_id": r5["referenced_review_id"]}
        graph = {"by_raw": {raw: {
            "pending_terminals": {pending["id"]: pending},
            "independent_reviews": {review["id"]: review},
            "accepted_terminals": {accepted["id"]: accepted},
            "blocked_attempts": {attempt["id"]: attempt},
            "remediation_records": {remediation["id"]: remediation},
        }}}
        result = contract.validate_disposition_chain([r0, r1, r2, r3, r4, r5], graph)
