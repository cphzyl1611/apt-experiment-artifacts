...
def execute_role_with_backend(*args: Any, **kwargs: Any) -> dict[str, Any]:
    """R3 production role adapter exposed through the contract module."""
    from tools.a2_role_runtime import execute_role_with_backend as _execute
...
def _universe_pairs(universe: dict[str, Any] | None) -> set[tuple[str, str]]:
    if not isinstance(universe, dict):
        raise ValueError("authenticated complete candidate universe is required")
    rows = universe.get("candidate_relations")
    if not isinstance(rows, list) or not rows:
        raise ValueError("authenticated candidate universe is missing")
    pairs = {(row.get("candidate_scoring_id"), row.get("relation_identity")) for row in rows if isinstance(row, dict)}
    if len(pairs) != len(rows) or any(not isinstance(c, str) or not isinstance(r, str) for c, r in pairs):
        raise ValueError("authenticated candidate universe is malformed")
    return pairs


def _require_authenticated_object(value: dict[str, Any] | None, name: str) -> dict[str, Any]:
    if not isinstance(value, dict) or not value:
        raise ValueError(f"authenticated {name} is required")
    return value


def _object_hash(value: dict[str, Any]) -> str:
    return sha256_bytes(project_canonical_json(value))


def _require_authenticated_id(value: dict[str, Any], field: str, name: str) -> str:
    identifier = value.get(field)
    if not isinstance(identifier, str) or not HEX64_RE.fullmatch(identifier):
        raise ValueError(f"authenticated {name} identity is missing")
    if canonical_object_id(value, field) != identifier:
        raise ValueError(f"authenticated {name} identity mismatch")
    return identifier


def _validate_frozen_schema(name: str, instance: dict[str, Any]) -> bool:
    schema_name = f"{name}.schema.json"
    candidates = (
        ROOT / "05_schemas" / schema_name,
        ROOT
        / "FA1B2de_Current86_BSO_A2_P0_P1_Execution_Contract_R4"
        / "05_schemas"
        / schema_name,
    )
    schema_path = next((path for path in candidates if path.is_file()), None)
    if schema_path is None:
        raise ValueError(f"frozen schema is unavailable: {name}")
    try:
        import jsonschema
        ...
        raise ValueError("jsonschema runtime is required for frozen object validation") from exc
    except jsonschema.exceptions.SchemaError as exc:
        raise ValueError(f"invalid frozen schema: {name}: {exc.message}") from exc
    except jsonschema.exceptions.ValidationError as exc:
        raise ValueError(f"schema validation failed for {name}: {exc.message}") from exc
...
def _validate_packet_references(
    packet: dict[str, Any],
    frozen_candidate_universe: dict[str, Any] | None,
    authenticated_proposal_input_bundle: dict[str, Any] | None,
    proposal: dict[str, Any] | None,
    primary_commitment: dict[str, Any] | None,
    verifier_commitment: dict[str, Any] | None,
    comparison: dict[str, Any] | None,
    authenticated_evidence_references: list[dict[str, Any]] | None,
) -> set[tuple[str, str]]:
    universe = _require_authenticated_object(frozen_candidate_universe, "complete candidate universe")
    bundle = _require_authenticated_object(
        authenticated_proposal_input_bundle, "proposal input bundle"
    )
    proposal = _require_authenticated_object(proposal, "proposal")
    primary = _require_authenticated_object(primary_commitment, "primary commitment")
    verifier = _require_authenticated_object(verifier_commitment, "verifier commitment")
    comparison = _require_authenticated_object(comparison, "verifier comparison")
    if not isinstance(authenticated_evidence_references, list) or not authenticated_evidence_references:
        raise ValueError("authenticated evidence references are required")
    raw = packet.get("raw_key")
    for name, value in (("candidate universe", universe), ("proposal input bundle", bundle), ("proposal", proposal), ("primary commitment", primary), ("verifier commitment", verifier), ("comparison", comparison)):
        if value.get("raw_key") != raw:
            raise ValueError(f"authenticated {name} raw_key mismatch")
    _validate_frozen_schema("complete_candidate_universe", universe)
    _validate_frozen_schema("proposal_input_bundle", bundle)
    _validate_frozen_schema("primary_commitment", primary)
    _validate_frozen_schema("verifier_commitment", verifier)
    _validate_frozen_schema("proposer_verifier_comparison", comparison)
    _require_authenticated_id(
        universe, "complete_candidate_universe_id", "complete candidate universe"
    )
    bundle_id = _require_authenticated_id(
        bundle, "proposal_input_bundle_id", "proposal input bundle"
    )
    primary_id = _require_authenticated_id(primary, "commitment_id", "primary commitment")
    verifier_id = _require_authenticated_id(verifier, "commitment_id", "verifier commitment")
    comparison_id = _require_authenticated_id(comparison, "comparison_id", "comparison")
    if packet.get("proposal_hash") != _object_hash(proposal):
        raise ValueError("human packet proposal hash mismatch")
    if packet.get("primary_commitment_id") != primary_id or packet.get("primary_commitment_hash") != _object_hash(primary):
        raise ValueError("human packet primary commitment binding mismatch")
    if packet.get("verifier_commitment_id") != verifier_id or packet.get("verifier_commitment_hash") != _object_hash(verifier):
        raise ValueError("human packet verifier commitment binding mismatch")
    comparison_hash = _object_hash(comparison)
    if packet.get("comparison_id") != comparison_id or packet.get("comparison_hash") != comparison_hash or packet.get("verifier_comparison_hash") != comparison_hash:
        raise ValueError("human packet comparison binding mismatch")
    universe_hashes = {
        packet.get("complete_candidate_universe_hash"),
        universe.get("complete_candidate_universe_hash"),
        bundle.get("complete_candidate_universe_hash"),
        primary.get("complete_candidate_universe_hash"),
        verifier.get("complete_candidate_universe_hash"),
    }
    if len(universe_hashes) != 1 or None in universe_hashes:
        raise ValueError("semantic complete candidate universe hash mismatch")
    relation_hashes = {
        universe.get("complete_candidate_relation_set_hash"),
        bundle.get("complete_candidate_relation_set_hash"),
        primary.get("complete_relation_set_hash"),
        verifier.get("complete_relation_set_hash"),
    }
    if len(relation_hashes) != 1 or None in relation_hashes:
        raise ValueError("complete candidate relation set hash mismatch")
    input_bundle_ids = {
        bundle_id,
        primary.get("proposal_input_bundle_id"),
        verifier.get("proposal_input_bundle_id"),
    }
    if len(input_bundle_ids) != 1:
        raise ValueError("proposal input bundle identity mismatch")
    proposed_pair = (packet.get("proposed_candidate_scoring_id"), packet.get("proposed_relation_identity"))
    for name, value in (("proposal", proposal), ("primary commitment", primary), ("verifier commitment", verifier)):
        if (value.get("selected_candidate_scoring_id"), value.get("selected_relation_identity")) != proposed_pair:
            raise ValueError(f"human packet proposed pair does not match authenticated {name}")
    required_comparison_assertions = (
        "same_input_bundle",
    ...
    if (
        comparison.get("primary_commitment_id") != primary_id
        or comparison.get("verifier_commitment_id") != verifier_id
        or any(comparison.get(field) is not True for field in required_comparison_assertions)
        or comparison.get("independent_commitment_comparison") != "PASS"
    ):
        raise ValueError("authenticated comparison does not bind an exact passing commitment pair")
    if primary.get("context_identity") == verifier.get("context_identity"):
        raise ValueError("primary and verifier context identities are not distinct")
    if primary.get("run_identity") == verifier.get("run_identity"):
        raise ValueError("primary and verifier run identities are not distinct")
    evidence_by_id: dict[str, dict[str, Any]] = {}
    for reference in authenticated_evidence_references:
        if not isinstance(reference, dict):
            raise ValueError("authenticated evidence reference is malformed")
        source_fact_id = reference.get("source_fact_id")
        if not isinstance(source_fact_id, str) or not HEX64_RE.fullmatch(source_fact_id) or source_fact_id in evidence_by_id:
            raise ValueError("authenticated evidence identity is malformed or duplicated")
        if reference.get("raw_key") != raw:
            raise ValueError("authenticated evidence raw_key mismatch")
        evidence_by_id[source_fact_id] = reference
    for side in ("raw", "candidate"):
        fact_ids = packet.get(f"{side}_side_evidence_fact_ids")
        embedded = packet.get(f"{side}_side_evidence_source_references")
        if not isinstance(fact_ids, list) or not isinstance(embedded, list) or len(fact_ids) != len(embedded):
            raise ValueError(f"human packet {side}-side evidence binding is missing")
        for fact_id, reference in zip(fact_ids, embedded):
            if evidence_by_id.get(fact_id) != reference:
                raise ValueError(f"human packet {side}-side evidence is not the exact authenticated reference")
    return _universe_pairs(universe)


def validate_human_packet(
    packet: dict[str, Any],
    frozen_candidate_universe: dict[str, Any] | None = None,
    proposal: dict[str, Any] | None = None,
    primary_commitment: dict[str, Any] | None = None,
    verifier_commitment: dict[str, Any] | None = None,
    comparison: dict[str, Any] | None = None,
    authenticated_evidence_references: list[dict[str, Any]] | None = None,
    *,
    authenticated_proposal_input_bundle: dict[str, Any] | None = None,
    proposal_input_bundle: dict[str, Any] | None = None,
) -> bool:
    if authenticated_proposal_input_bundle is not None and proposal_input_bundle is not None and authenticated_proposal_input_bundle != proposal_input_bundle:
        raise ValueError("conflicting authenticated proposal input bundle references")
    bundle = authenticated_proposal_input_bundle if authenticated_proposal_input_bundle is not None else proposal_input_bundle
    universe_pairs = _validate_packet_references(packet, frozen_candidate_universe, bundle, proposal, primary_commitment, verifier_commitment, comparison, authenticated_evidence_references)
    ...
    mapping_pairs = set(zip(candidates, relations))
    if mapping_pairs != universe_pairs or len(mapping_pairs) != len(universe_pairs) or len(mapping) != len(universe_pairs):
        raise ValueError("human packet mapping is not the exact authenticated candidate universe")
...
def validate_human_decision(
    ...
    comparison: dict[str, Any] | None = None,
    *,
    human_packet: dict[str, Any] | None = None,
    frozen_candidate_universe: dict[str, Any] | None = None,
    authenticated_proposal_input_bundle: dict[str, Any] | None = None,
    proposal: dict[str, Any] | None = None,
    primary_commitment: dict[str, Any] | None = None,
    ...
    authenticated_packet = _require_authenticated_object(human_packet, "immutable human packet")
    if packet != authenticated_packet:
        raise ValueError("human decision packet is not the exact authenticated immutable packet")
    validate_human_packet(
        ...
        authenticated_proposal_input_bundle=authenticated_proposal_input_bundle,
        proposal_input_bundle=proposal_input_bundle,
    )
    packet_id = _require_authenticated_id(authenticated_packet, "human_packet_id", "human packet")
    if decision.get("human_packet_id") != packet_id or decision.get("human_packet_hash") != _object_hash(authenticated_packet):
        raise ValueError("human decision does not bind the authenticated immutable packet")
    if decision.get("raw_key") != packet.get("raw_key"):
        raise ValueError("human decision raw_key mismatch")
    _require_authenticated_id(decision, "human_decision_record_id", "human decision")
    ...
    if action == "REJECT_PROPOSAL_AND_SELECT_OTHER_EXISTING_CANDIDATE":
        ...
        if (selected_candidate, selected_relation) == (packet.get("proposed_candidate_scoring_id"), packet.get("proposed_relation_identity")):
            raise ValueError("REJECT_SELECT cannot select the original proposal")
...
def validate_owner_terminal(
    ...
    *,
    ...
    verifier_commitment: dict[str, Any] | None = None,
    authenticated_evidence_references: list[dict[str, Any]] | None = None,
    alternative_verification: dict[str, Any] | None = None,
    proposal_input_bundle: dict[str, Any] | None = None,
) -> bool:
    ...
    human_decision = _require_authenticated_object(human_decision, "human decision")
    human_packet = _require_authenticated_object(human_packet, "human packet")
    validate_human_decision(
        human_packet,
        human_decision,
        comparison,
        human_packet=human_packet,
        frozen_candidate_universe=frozen_candidate_universe,
        authenticated_proposal_input_bundle=authenticated_proposal_input_bundle,
        proposal=proposal,
        primary_commitment=primary_commitment,
        verifier_commitment=verifier_commitment,
        authenticated_evidence_references=authenticated_evidence_references,
        proposal_input_bundle=proposal_input_bundle,
    )
    decision_id = _require_authenticated_id(human_decision, "human_decision_record_id", "human decision")
    packet_id = _require_authenticated_id(human_packet, "human_packet_id", "human packet")
    if record.get("human_decision_record_id") != decision_id or record.get("human_packet_id") != packet_id or record.get("raw_key") != human_packet.get("raw_key"):
        raise ValueError("owner terminal does not bind exact authenticated packet and decision")
    if record.get("human_action") != human_decision.get("human_action"):
        raise ValueError("owner terminal action does not equal authenticated human decision")
    if (record.get("selected_owner_candidate_scoring_id"), record.get("selected_relation_identity")) != (human_decision.get("human_selected_candidate_scoring_id"), human_decision.get("human_selected_relation_identity")):
        raise ValueError("owner terminal selection does not equal validated human decision")
    if (record.get("selected_owner_candidate_scoring_id"), record.get("selected_relation_identity")) not in _universe_pairs(frozen_candidate_universe):
        raise ValueError("owner terminal selection is outside authenticated candidate universe")
    if record.get("human_action") == "REJECT_PROPOSAL_AND_SELECT_OTHER_EXISTING_CANDIDATE":
        verification = _require_authenticated_object(alternative_verification, "alternative verification")
        verification_id = _require_authenticated_id(verification, "alternative_verification_id", "alternative verification")
        selected = (record.get("selected_owner_candidate_scoring_id"), record.get("selected_relation_identity"))
        if verification.get("raw_key") != record.get("raw_key") or verification.get("status") != "PASS" or (verification.get("candidate_scoring_id"), verification.get("relation_identity")) != selected:
            raise ValueError("alternative verification does not bind exact selected alternative PASS")
        if record.get("fresh_alternative_verification") != "PASS" or record.get("fresh_alternative_verification_id") != verification_id or human_decision.get("fresh_alternative_verification_id") != verification_id:
            raise ValueError("owner and decision do not reference the exact alternative verification")
    return True
...
ALLOWED_INVALIDATION_REASON_CLASSES = {
    "SUBSTANTIVE_TERMINAL_REJECTION",
    "SUBSTANTIVE_REVIEW_REJECTION",
...
def _resolve_reference(references: dict[str, Any], collection: str, identifier: str | None) -> dict[str, Any] | None:
    if identifier is None:
        return None
    values = references.get(collection, {})
    ...
    if isinstance(values, list):
        for value in values:
            if isinstance(value, dict):
                ids = [item for key, item in value.items() if key == "id" or key.endswith("_id")]
                if identifier in ids:
                    return value
    return None


def _resolve_authenticated_reference_graph(
    graph: dict[str, Any], raw: str, collection: str, identifier: str | None
) -> dict[str, Any]:
    if not isinstance(identifier, str) or not HEX64_RE.fullmatch(identifier):
        raise ValueError(f"{collection} reference identity is missing")
    by_raw = graph.get("by_raw")
    if not isinstance(by_raw, dict):
        raise ValueError("authenticated reference graph requires by_raw")
    raw_graph = by_raw.get(raw)
    if not isinstance(raw_graph, dict):
        raise ValueError("authenticated reference graph is missing exact raw")
    values = raw_graph.get(collection)
    if not isinstance(values, dict):
        raise ValueError(f"authenticated reference graph is missing {collection}")
    value = values.get(identifier)
    if not isinstance(value, dict):
        raise ValueError(f"authenticated reference graph cannot resolve {collection}")
    if _reference_identifier(value, identifier) != identifier:
        raise ValueError(f"authenticated {collection} object identity mismatch")
    if value.get("raw_key") != raw:
        raise ValueError(f"authenticated {collection} raw identity mismatch")
    return value


def _resolve_disposition_record_references(
    graph: dict[str, Any], record: dict[str, Any], prior: dict[str, Any]
) -> dict[str, Any]:
    raw = record.get("raw_key")
    if not isinstance(raw, str):
        raise ValueError("disposition raw_key is required")
    references: dict[str, Any] = {"current_prior_disposition": prior}
    typ = record.get("disposition_type")
    if typ in {"PENDING_TERMINAL_INVALIDATED", "TERMINAL_ACCEPTED"}:
        references["current_pending_terminal"] = _resolve_authenticated_reference_graph(
            graph, raw, "pending_terminals", record.get("referenced_pending_terminal_id")
        )
    if typ == "PENDING_TERMINAL_INVALIDATED":
        references["blocking_review"] = _resolve_authenticated_reference_graph(
            graph, raw, "independent_reviews", record.get("referenced_review_id")
        )
    elif typ == "TERMINAL_ACCEPTED":
        references["passing_independent_review"] = _resolve_authenticated_reference_graph(
            graph, raw, "independent_reviews", record.get("referenced_review_id")
        )
        references["accepted_terminal"] = _resolve_authenticated_reference_graph(
            graph, raw, "accepted_terminals", record.get("referenced_accepted_terminal_id")
        )
    elif typ == "REMEDIATION_RESTARTED":
        references["preceding_blocked_attempt"] = _resolve_authenticated_reference_graph(
            graph, raw, "blocked_attempts", record.get("referenced_attempt_id")
        )
        references["remediation_record"] = _resolve_authenticated_reference_graph(
            graph, raw, "remediation_records", record.get("remediation_reference")
        )
    return references


def _reference_identifier(obj: dict[str, Any], fallback_field: str) -> Any:
    if obj.get("id") is not None:
        return obj.get("id")
    if obj.get(fallback_field) is not None:
        return obj.get(fallback_field)
    suffix = fallback_field.removeprefix("referenced_")
    return obj.get(suffix)


def validate_disposition_record(record: dict[str, Any], references: dict[str, Any] | None = None) -> bool:
    typ = record.get("disposition_type")
    ...
    if typ == "PENDING_TERMINAL_INVALIDATED":
        ...
        if record.get("reason_class") not in ALLOWED_INVALIDATION_REASON_CLASSES:
            raise ValueError("PENDING_TERMINAL_INVALIDATED requires an allowed invalidation reason_class")
    elif typ == "TERMINAL_ACCEPTED":
        _require_lineage_id(record, "referenced_pending_terminal_id")
        _require_lineage_id(record, "referenced_review_id")
    ...
    exact_lineage_types = {"PENDING_TERMINAL_INVALIDATED", "TERMINAL_ACCEPTED", "REMEDIATION_RESTARTED"}
    if typ in exact_lineage_types and not isinstance(references, dict):
        raise ValueError(f"{typ} requires authenticated exact-lineage references")
    if references is not None and typ in exact_lineage_types:
        raw = record.get("raw_key")
        if not isinstance(raw, str):
            raise ValueError("disposition raw_key is required")
        prior = references.get("current_prior_disposition")
        prior_id = _reference_identifier(prior, "disposition_id") if isinstance(prior, dict) else None
        if not isinstance(prior, dict) or prior_id != record.get("prior_disposition_id") or prior.get("raw_key") != raw:
            raise ValueError("prior disposition is not the exact authenticated current head")
        prior_sequence = prior.get("disposition_sequence")
        if not isinstance(prior_sequence, int) or record.get("disposition_sequence") != prior_sequence + 1:
            raise ValueError("disposition sequence does not immediately follow authenticated current head")
        if typ in {"PENDING_TERMINAL_INVALIDATED", "TERMINAL_ACCEPTED"}:
            current = references.get("current_pending_terminal")
            if not isinstance(current, dict) or _reference_identifier(current, "referenced_pending_terminal_id") != record.get("referenced_pending_terminal_id") or current.get("raw_key") != raw:
                raise ValueError("referenced pending terminal is not the exact current pending terminal")
            if prior.get("next_state") != "PENDING_REVIEW" or prior.get("referenced_pending_terminal_id") != record.get("referenced_pending_terminal_id"):
                raise ValueError("prior disposition is not the current PENDING_REVIEW head for the exact pending terminal")
        if typ == "PENDING_TERMINAL_INVALIDATED":
            review = references.get("blocking_review")
            if not isinstance(review, dict) or _reference_identifier(review, "referenced_review_id") != record.get("referenced_review_id") or review.get("raw_key") != raw:
                raise ValueError("blocking review is not the exact referenced same-raw review")
            if review.get("referenced_pending_terminal_id") != record.get("referenced_pending_terminal_id") or review.get("status") not in {"BLOCKING", "REJECTED", "FAIL_CLOSED", "SUBSTANTIVE_REJECTION"}:
                raise ValueError("review is not a substantive block of the exact current pending terminal")
        if typ == "TERMINAL_ACCEPTED":
            review = references.get("passing_independent_review")
            accepted = references.get("accepted_terminal")
            if not isinstance(review, dict) or _reference_identifier(review, "referenced_review_id") != record.get("referenced_review_id") or review.get("raw_key") != raw or review.get("status") != "PASS":
                raise ValueError("passing independent review is not the exact referenced same-raw PASS")
            if review.get("referenced_pending_terminal_id") != record.get("referenced_pending_terminal_id"):
                raise ValueError("passing review does not reference the exact current pending terminal")
            if not isinstance(accepted, dict) or _reference_identifier(accepted, "referenced_accepted_terminal_id") != record.get("referenced_accepted_terminal_id") or accepted.get("raw_key") != raw:
                raise ValueError("accepted terminal reference does not resolve")
            if accepted.get("referenced_pending_terminal_id") != record.get("referenced_pending_terminal_id") or accepted.get("referenced_review_id") != record.get("referenced_review_id") or review.get("approved_terminal_id") != record.get("referenced_accepted_terminal_id"):
                raise ValueError("review does not approve referenced accepted terminal")
        if typ == "REMEDIATION_RESTARTED":
            attempt = references.get("preceding_blocked_attempt")
            remediation = references.get("remediation_record")
            if prior.get("next_state") != "BLOCKED_ATTEMPT":
                raise ValueError("preceding disposition is not the exact blocked current head")
            if not isinstance(attempt, dict) or _reference_identifier(attempt, "referenced_attempt_id") != record.get("referenced_attempt_id") or attempt.get("raw_key") != raw or attempt.get("state") != "BLOCKED_ATTEMPT" or attempt.get("disposition_id") != prior_id:
                raise ValueError("preceding attempt is not the exact blocked attempt")
            if not isinstance(remediation, dict) or _reference_identifier(remediation, "remediation_reference") != record.get("remediation_reference") or remediation.get("raw_key") != raw or remediation.get("authorizes_restart_of") != prior_id or remediation.get("authorizes_restart_of_attempt_id") != record.get("referenced_attempt_id"):
                raise ValueError("remediation does not authorize restart of exact blocked state")
    if canonical_object_id(record, "disposition_id") != record.get("disposition_id"):
        raise ValueError("disposition ID mismatch")
    return True


def validate_disposition_chain(records: list[dict[str, Any]], references: dict[str, Any] | None = None) -> dict[str, Any]:
    if not records:
    ...
    for index, record in enumerate(records):
        if record.get("disposition_type") in {"PENDING_TERMINAL_INVALIDATED", "TERMINAL_ACCEPTED", "REMEDIATION_RESTARTED"}:
            if index == 0:
                raise ValueError("exact disposition transition cannot be the initial chain record")
            if not isinstance(references, dict) or "by_raw" not in references:
                raise ValueError("exact disposition chain requires authenticated reference graph")
            record_references = _resolve_disposition_record_references(
                references, record, records[index - 1]
            )
        else:
            record_references = None
        validate_disposition_record(record, record_references)
        typ = record.get("disposition_type")
...
def reconstruct_partitions(
    raw_keys: list[str] | tuple[str, ...],
    chains: dict[str, list[dict[str, Any]]],
    authenticated_reference_graph: dict[str, Any],
) -> dict[str, list[str]]:
    expected = set(raw_keys)
    if set(chains) != expected:
        raise ValueError("partition input is not exact Current86")
    if not isinstance(authenticated_reference_graph, dict) or not isinstance(
        authenticated_reference_graph.get("by_raw"), dict
    ):
        raise ValueError("partition reconstruction requires authenticated reference graph")
    ...
    for raw in raw_keys:
        state = validate_disposition_chain(
            chains[raw], authenticated_reference_graph
        )["current_state"]
...
def materialize(root: Path = ROOT, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=".a2-contract-staging-", dir=output_dir.parent) as staging_name:
        ...
        initial_reference_graph = {"by_raw": {raw: {
            "pending_terminals": {},
            "independent_reviews": {},
            "accepted_terminals": {},
            "blocked_attempts": {},
        ...
        partitions = reconstruct_partitions(
            auth.raw_keys, initial_chains, initial_reference_graph
        )
